...
/**
 * Estrelas e tag do último release. Resolvido no servidor: buscar isso do
 * navegador gastaria o limite da API do GitHub por IP do visitante, além de
 * exigir uma origem externa liberada na CSP.
 */
export async function findRepoStats(repoName) {
    const repo = resolveRepo(repoName);
    if (!repo) return null;
    ...
    const [repoResponse, releaseResponse] = await Promise.all([
        ...
        fetch(`https://api.github.com/repos/${repo}/releases/latest`, options).catch(() => null),
    ]);
    ...
    if (releaseResponse?.ok) {
        ...
        releaseTag = data.tag_name || data.name || null;
    }

    return stars === null && releaseTag === null ? null : { stars, releaseTag };
}
